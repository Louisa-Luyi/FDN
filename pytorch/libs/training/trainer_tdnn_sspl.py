...
class SimpleTrainer(_BaseTrainer):
    ...
    def train_one_batch(self, batch, target_batch, alpha):
        ...
        lang_labels = torch.cat((tlang_labels, slang_labels), 0)

        inputs, targets, lang_labels = inputs.cuda(), targets.cuda(), lang_labels.cuda()
        target_data, ttargets = target_data.cuda(), ttargets.cuda()
        optimizer.zero_grad()

        loss_c, loss_lang, loss_mi = model.get_loss(model_forward(target_data, inputs, alpha), ttargets, targets, lang_labels)
        loss = loss_c + loss_lang + loss_mi
...
#        model.do_gradient_ops()
        ...
        accuracy = model.get_accuracy(ttargets) if self.params["compute_accuracy"] else None

        return loss.item(), accuracy, loss_c.item(), loss_lang.item(), loss_mi.item()
...
#            batch_size = self.params["batch_size"]
            ...
            for this_epoch in range(start_epoch, epochs):
                ...
                for this_iter, batch in enumerate(data.train_loader):
                    ...
                    p = float(this_iter + this_epoch * float(data.num_batch_train)/2.0) / epochs /float(data.num_batch_train)/2.0
                    p = 2. / (1. + np.exp(-10 * p)) - 1
...
#                    self.tsne_plot(extracted_lang_features.squeeze().detach().cpu().numpy(), labels, 'lang_spk.pdf')
                    ...
                    loss, acc, loss_c, loss_lang, loss_mi = self.train_one_batch(batch, target_batch, valid_alpha)
#                    print("d1: ", model.discriminator1.weight.grad)
#                    print(model.feature_extractor.spk_tdnn6.affine.weight.grad)

                    if this_iter % 100 == 0:
                        print("loss spk: ", loss_c)
                        print("loss lang: ", loss_lang)
                        print("loss mi: ", loss_mi)
                    ...
                    if lr_scheduler.name == "reduceP" and lr_scheduler.is_reduce_point(self.training_point):
                        assert target_data.valid_loader is not None
                        valid_loss, valid_acc = self.compute_validation(target_data.valid_loader, valid_alpha)
                        lr_scheduler_params["valid_metric"] = (valid_loss, valid_acc)
                        valid_computed = True

                    if utils.is_main_training():
                        if valid_computed or (target_data.valid_loader and self.reporter.is_report(self.training_point)):
