...
class SimpleTrainer(_BaseTrainer):
    ...
    def train_one_batch(self, batch, target_batch, lang_alpha, dom_alpha):
        ...
        if self.params["nan_debug"]:
            ...
            inputs, targets, slang_labels, sdom_labels = batch

        target_data, _, tlang_labels, tdom_labels = target_batch

        lang_labels = torch.cat((slang_labels, tlang_labels), 0).cuda()
        dom_labels = torch.cat((sdom_labels, tdom_labels), 0).cuda()

        inputs, targets, target_data = inputs.cuda(), targets.cuda(), target_data.cuda()
        optimizer.zero_grad()

        loss_c, loss_lang, loss_dom, mi_lang, mi_dom = model.get_loss(model_forward(inputs, target_data, lang_alpha, dom_alpha),
                                                                      targets, lang_labels, dom_labels)
        loss = loss_c + loss_lang + loss_dom + mi_lang + mi_dom
...
#        model.do_gradient_ops()
        ...
        return loss.item(), accuracy, loss_c.item(), loss_lang.item(), loss_dom.item(), mi_lang.item(), mi_dom.item()
    ...
    def compute_validation(self, data_loader, valid_alpha):
        ...
        with torch.no_grad():
            for this_data in data_loader:
                ...
                loss += model.get_closs(model_forward(inputs, inputs, valid_alpha, valid_alpha), targets).item() * len(targets)
...
#                    self.tsne_plot(extracted_lang_features.squeeze().detach().cpu().numpy(), labels, 'lang_spk.pdf')
                    ...
                    loss, acc, loss_c, loss_lang, loss_dom, mi_lang, mi_dom = self.train_one_batch(batch, target_batch, valid_alpha, alpha)
